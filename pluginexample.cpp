...
QString PluginExample::PluginCommand(const QString &command, const QString &value){
    ...
    // 新增：支持 API 打开插件（触发 Robot Pilot UI）
    else if (command.compare("OPEN_PLUGIN", Qt::CaseInsensitive) == 0) {
        qDebug() << "API 命令：打开 Example Plugin UI";
        if (value == "open" || value.isEmpty()) {  // 支持 value="open" 或空
            callback_robotpilot();  // 调用现有逻辑打开 FormRobotPilot
            return "SUCCESS";  // 返回字符串反馈给客户端（可选，C++ 接口允许）
        }
        else {
            qDebug() << "无效 value: " << value;
...
void outputMessage(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
    ...
    // 输出信息至文件中（读写、追加形式）
    QFile file("D:\\cw\\robodklog\\log.txt");
